...
namespace CostEstimate.Controllers.MyRequestMold
{
    public class MyRequestMoldController : Controller
    {
        private LAMP _LAMP;
        private HRMS _HRMS;
        private IT _IT;
        ...
        public MyRequestMoldController(LAMP lamp, HRMS hrms, IT it, MK mk, MOLD mold, CacheSettingController cacheController, FunctionsController callfunction)
        {
            _LAMP = lamp;
            _HRMS = hrms;
            _IT = it;
        ...
        public IActionResult Index(Class @class)
        {
            ...
            @class._ListViewceMastModifyRequest = _MK._ViewceMastModifyRequest.Where(x => x.mfEmpCodeRequest == EmpCode).OrderBy(x => x.mfStep).ThenBy(x => x.mfIssueDate).ToList();
