...
namespace CostEstimate.Controllers.ApprovalMoldOther
{
    public class ApprovalMoldOtherController : Controller
    {
        ...
        public IActionResult Index(Class @class)
        {
            ...
            @class._ListViewceMastMoldOtherRequest = _MK._ViewceMastMoldOtherRequest.Where(x => x.mrEmpCodeApprove == user_id && x.mrStep != 8).ToList();

            @class._ListViewceMastWorkingTimeRequest = _MK._ViewceMastWorkingTimeRequest.Where(x => x.wrEmpCodeApprove == user_id && x.wrStep != 4).ToList();
            ...
            @class._ListViewceMastMaterialRequest = _MK._ViewceMastMaterialRequest.Where(x => x.mrEmpCodeApprove == user_id && x.mrStep != 4).ToList();
            @class._ListViewceMastMaterialRequestDetail = getceMastMaterialRequestDetail(@class._ListViewceMastMaterialRequest);

            @class._ListViewceMastToolGRRequest = _MK._ViewceMastToolGRRequest.Where(x => x.trEmpCodeApprove == user_id && x.trStep != 4).ToList();
            @class._ListViewceMastToolGRRequestDetail = getceMastToolGRRequestDetail(@class._ListViewceMastToolGRRequest);

            @class._ListViewceMastInforSpacMoldRequest = _MK._ViewceMastInforSpacMoldRequest.Where(x => x.irEmpCodeApprove == user_id && x.irStep != 4).ToList();
