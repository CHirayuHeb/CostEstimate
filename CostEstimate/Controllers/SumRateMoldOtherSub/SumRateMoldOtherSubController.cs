...
namespace CostEstimate.Controllers.SumRateMoldOtherSub
{
    public class SumRateMoldOtherSubController : Controller
    {
        private LAMP _LAMP;
        private HRMS _HRMS;
        private IT _IT;
        ...
        public SumRateMoldOtherSubController(LAMP lamp, HRMS hrms, IT it, MK mk, MOLD mold, CacheSettingController cacheController, FunctionsController callfunction)
        {
            _LAMP = lamp;
            _HRMS = hrms;
            _IT = it;
        ...
        public IActionResult Index(Class @class, string Docno, int vProcess)
        {
            try
            {
                ...
                @class._ListViewceItemMaterialRequestPartName = _MK._ViewceItemMaterialRequestPartName.Where(x => x.mpDocumentNoSub == @class._ViewceMastMaterialRequest.mrDocumentNoSub && x.mpNoProcess == vProcess && x.mpItem.Contains("SubMaker")).ToList();

                //select CE mast cost


                //select CE mast cost
                @class._ListGroupViewceMastChartRateOtherReport = getListGroupViewceMastChartRateOtherReport(Docno, vProcess, "start-try0");
                @class._ListGroupViewceMastChartRateOtherReportShip = getListGroupViewceMastChartRateOtherReport(Docno, vProcess, "try0-ship");
        ...
        public List<GroupViewceMastChartRateOtherReport> getListGroupViewceMastChartRateOtherReport(string Docno, int vProcess, string wpshipment)
        {
            ...
            var ceWorkingTimePartName = _MK._ViewceItemWorkingTimePartName.Where(x => x.wpDocumentNoSub == docWokingTime && x.wpNoProcess == vProcess && x.wpshipment == wpshipment).ToList();
            ...
            */
                ...
                if (RuleChartRate != null)
                {
                    ...
                    foreach (var part in manParts)
                    {
                        ...
                        if (lmanParts.Count() > 1)
                        {
                            ...
                            var vceItemWorking = _MK._ViewceItemWorkingTimePartName.Where(x => x.wpDocumentNoSub == docWokingTime && x.wpNoProcess == vProcess && x.wpProcessName == lmanParts[0].ToString() && x.wpshipment == wpshipment).ToList();
            ...
            //wpshipment
            List<GroupViewceMastChartRateOtherReport> _ListGroupViewceMastChartRateOtherReport = new List<GroupViewceMastChartRateOtherReport>();
            _ListGroupViewceMastChartRateOtherReport = @class._ListViewDetailceMastChartRateOtherReport.GroupBy(p => p.crGroupName).Select(g => new GroupViewceMastChartRateOtherReport
            {
                GroupName = g.Key.Trim(),
                DetailceMastChartRateOtherReport = g.ToList()
            }).ToList();

            return _ListGroupViewceMastChartRateOtherReport;
