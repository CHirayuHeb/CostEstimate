...
namespace CostEstimate.Controllers.AddMProcess
{
    public class AddMProcessController : Controller
    {
        private LAMP _LAMP;
        private HRMS _HRMS;
        private IT _IT;
        ...
        public AddMProcessController(LAMP lamp, HRMS hrms, IT it, MK mk, MOLD mold, CacheSettingController cacheController, FunctionsController callfunction)
        {
            _LAMP = lamp;
            _HRMS = hrms;
            _IT = it;
        ...
        public IActionResult Index(Class @class)
        {
            ...
            @class._ListceMastProcess = _MK._ViewceMastProcess.Where(x => x.mpType == "MoldModify").OrderBy(z => z.mpGroupName.Trim()).ToList();

            return View(@class);
        }

        public ActionResult DeleteMasterMProcess(string processGroup, string processName)
        {
            try
            {
                //cerunCostPalnning
                ViewceMastProcess vRun = _MK._ViewceMastProcess.Where(x => x.mpGroupName == processGroup && x.mpProcessName == processName && x.mpType == "MoldModify").FirstOrDefault();
                if (vRun != null)
                {
        ...
        [HttpPost]
        public ActionResult AddMasterMProcess(Class @class)
        {
            string config = "S";
            string[] vRunCostNo;
            string[] vSaveCost;
            string msg = "Save Master Mold Process success!!";
                ...
                try
                {
                    if (@class._ViewceMastProcess.mpNo > 0)
                    {
                        ...
                        if (_viewceMastProcess != null)
                        {
                            ...
                            _viewceMastProcess.mpType = "MoldModify";
